...
public class ProductDAO extends AbstractDAO<Product> {
			...
			public List<Product> extractData(ResultSet rs) throws SQLException, DataAccessException {
				while (rs.next()) {
					list.add(new Product(rs.getInt("p.id"), rs.getString("p.name"),
							...
							rs.getString("detail"), rs.getString("picture"), new User(rs.getInt("u.id"),
									rs.getString("username"), rs.getString("fullname"), rs.getString("avatar")),
	...
	public int save(Product product) {
		String sql = "INSERT INTO products(name,catId,description,detail,picture,userId) VALUES (?,?,?,?,?,?)";
		return jdbcTemplate.update(sql, product.getName(), product.getCat().getId(), product.getDescription(),
				product.getDetail(), product.getPicture(), product.getUser().getId());
	}
	...
	public int updatePicture(Product product) {
		String sql = "UPDATE products SET picture = ? WHERE id = ?";
		return jdbcTemplate.update(sql, product.getPicture(), product.getId());
	}
